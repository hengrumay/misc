...
class Config:
    ...
    @property
    def allow_auto_esign(self) -> bool:
        """DEMO-ONLY opt-in: unattended auto-e-sign of eval-passing specs in a
        ``bundle run``. Absent/false => specs are LEFT for human review + e-sign
        in the app (the GxP control). The demo config ships true so an unattended
        demo reproduces end-to-end; it MUST be false for any real/regulated
        deployment. Honors the ``ADS_ALLOW_AUTO_ESIGN`` env override (string)."""
        v = self.get("allow_auto_esign", False)
        if isinstance(v, bool):
            return v
        return str(v).strip().lower() in ("1", "true", "yes", "on")
